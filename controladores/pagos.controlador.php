...
class ControladorPagos{ 
	...
	static public function ctrMostrarPagos($item, $datos, $orden){

		$tabla = "pagos";

		$respuesta = ModeloPagos::mdlMostrarPagos($tabla, $item, $datos, $orden);
	...
	static public function ctrCrearPago(){
		...
		}else if(isset($_POST["nuevoPagoCompra"]) || isset($_POST["nuevoPagoGasto1"])){
			// echo "ola";
			if((preg_match('/^[0-9]+$/', $_POST["nuevoPagoCompra"]) && ($_POST["nuevoPagoCompra"] > 0)) || (preg_match('/^[0-9]+$/', $_POST["nuevoPagoGasto1"]) && ($_POST["nuevoPagoGasto1"] > 0))){

				$tabla = "pagos";

				if (isset($_POST["nuevoPagoCompra"])) {

					$datos = array("tipo_pago"=>$_POST["tipo_pago"],
							   "id_pago" =>$_POST["idCompra"],
							   "id_vendedor"=>$_POST["idVendedor"],
							   "pago"=>$_POST["nuevoPagoCompra"],
							   "total" => $_POST["nuevoTotalPago"]);

					$location = "compras";
				}


				if (isset($_POST["nuevoPagoGasto1"])) {
					$datos = array("tipo_pago"=>$_POST["tipoPago"],
							   "id_pago" =>$_POST["idPagoGasto"],
							   "id_vendedor"=>$_POST["idVendedor"],
							   "pago"=>$_POST["nuevoPagoGasto1"],
							   "total" => $_POST["nuevoTotalPago"]);
				...
				echo'<script>
					...
					swal({
						  type: "success",
						  title: "Este item ha sido pagado al 100%",
						  showConfirmButton: true,
						  confirmButtonText: "Cerrar"
						  }).then(function(result){
									if (result.value) {
